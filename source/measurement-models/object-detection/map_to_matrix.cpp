...
int*** matrix_from_file(int target_batch_height, int target_batch_width, string map_filename){
    int batch_ height = height_conversion(target_batch_height);
    cout << "new_height " << batch_height << '\n';
    int batch_width = width_conversion(target_batch_width);
    cout << "new_width " << batch_width << '\n';

    int num_rows = MAX_HEIGHT/batch_height;
    int num_cols = MAX_WIDTH/batch_width;

    int*** matrix = Create3D<int>(num_rows, num_cols, N_CLASSES);
